...
fn problem_3_7_thread_first_mutex(
    inner_barrier: Arc<Semaphore>,
    outer_barrier: Arc<Semaphore>,
    count: Arc<Mutex<i64>>,
    thread_index: i64,
    thread_count: i64,
) -> JoinHandle<()> {
    //let sem_clones: Vec<Arc<Semaphore>> = sems.iter().map(|sem| Arc::clone(&sem)).collect();
    return thread::spawn(move || {
        outer_barrier.acquire();
        outer_barrier.release();
        println!("Thread {thread_index} rendezvous");
        let mut current_count = count.lock().unwrap();
        *current_count += 1;
        if *current_count == thread_count {
            inner_barrier.release();
        }
        drop(current_count); // Missing this piece, this was frustrating
        inner_barrier.acquire();
        println!("Thread {thread_index} critical point");
        inner_barrier.release();
        outer_barrier.acquire();
    });
...
fn problem_3_7() {
    let thread_count = 4;
    let inner_barrier = Arc::new(lbs::Semaphore::new(0));
    let outer_barrier = Arc::new(lbs::Semaphore::new(thread_count));
    let count = Arc::new(Mutex::new(0));
    let mut handles = Vec::new();
